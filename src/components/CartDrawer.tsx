  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Backdrop */}
          <motion.div className="absolute inset-0 bg-foreground/80 backdrop-blur-xl" onClick={closeCart} />
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border p-6">
              <h2 className="font-display text-2xl text-foreground tracking-wide">YOUR CART</h2>
              <motion.button
                onClick={closeCart}
                className="rounded-full bg-surface/75 p-2 text-foreground transition-colors hover:bg-surface-hover"
                      ...
                      <div className="flex flex-1 flex-col">
                        <div className="flex items-start justify-between gap-2">
                          <Link
                            to={`/product/${item.slug}`}
                            onClick={closeCart}
                            className="font-display text-base text-foreground tracking-wide transition-colors hover:text-primary"
                          >
                            {item.name}
                          </Link>
                          <button
